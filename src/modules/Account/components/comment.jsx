...
export default function Comment({feed}) {
 ...
 const commentList=  feed?.comments.sort((a, b) => b.createdAt?.seconds - a.createdAt?.seconds);

//  console.log(com,"commmmmm")
  ...
  return (
    ...
    <div className='flex flex-col pt-4 pb-2 w-full space-y-4'>
       { commentList?.slice(0,`${trigger?feed?.comments?.length :3}`)?.map((comment)=>{
