...
const CollaborationScreen = () => {
  ...
  const getCollaboration = async () => {
    ...
    axios
      .get(url + "/collaboration/collabforcapture/" + collaboration_id)
      .then((res) => {
              ...
              {collaboration._doc.is_public ? (
                <div className="bg-white lg:w-[70vw] rounded-[20px] shadow-lg py-[30px] px-[40px]  ">
                  ...
                  {collaboration._doc.date && (
                    <div className="text-center font-[500] mt-[5px] underline text-[#114369] ">
                      Date:{" "}
                      {moment(collaboration._doc?.date).format("DD MMMM,YYYY")}
                    </div>
                  )}
                  ...
                  <div className="text-center  text-[19px] font-bold  space-x-4 md:flex justify-center mt-4 text-[#114369] ">
                    <Button
                      ...
                      onClick={() => {
                        if (user) {
                          navigate(
                            "/collaboration/contact-capture/" +
                              collaboration._doc._id
                          );
                        } else
                          navigate(
                            "/auth/login?collaboration_id=" +
                              collaboration._doc._id
                          );
                      }}
                    >
