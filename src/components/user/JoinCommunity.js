...
const JoinCommunity = () => {
  ...
  const acceptInvite = async () => {
    ...
    if (user.email !== d.data.reciever) {
      setMsg(
        "Email on invite does not match user email address, please log out from the account in session"
      );
      return;
    }
  ...
  return (
    <div className="md:flex justify-center md:mt-[2vw] mt-[4vw] ">
        ...
        <div>
          ...
          <div className="bg-white mt-5 lg:w-[40vw] md:w-[70vw]  shadow-lg md:px-20 px-4 py-10 rounded-lg">
            ...
            <div className="my-4 text-[12px] text-center">
              Or{" "}
              <span
                className="underline text-[#24A0FD] text-xs font-bold cursor-pointer "
                onClick={() => {
                  setStep(1);
                }}
              >
      ...
      {step === 2 && (
        <div>
          <div className="text-red-500 text-xs font-semibold text-center mb-4">
            {" "}
            {msg && (
              <div>
                {" "}
